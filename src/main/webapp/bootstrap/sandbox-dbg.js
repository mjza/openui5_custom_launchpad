...
(function () {
	...
	sap.ushell = sap.ushell || {}; // eslint-disable-line

	function _getObject(sName, iNoCreates, oContext) {
		var _iNoCreates = iNoCreates;
		if (isNaN(iNoCreates) || iNoCreates < 0) {
			_iNoCreates = 0;
		}
		...
		try {
			for (var i = 0; i < aNames.length; i++) {
				if (oObj[aNames[i]]) {
					oObj = oObj[aNames[i]];
				} else if(i < aNames.length - _iNoCreates){ // creation part
	...
	function getUriParameters(sParamName) {
		try {
			var queryString = window.location.search,
				urlParams = new URLSearchParams(queryString);
			return urlParams.get(sParamName);
		} catch (e) {
			return null;
		}
	}
	...
	function adjustApplicationConfiguration(oConfig) {
		...
		function fnGetApplicationKeys(oCfg) {
			...
			for (sApplicationKey in oCfg.applications) {
				...
				if (oCfg.applications.hasOwnProperty(sApplicationKey) && sApplicationKey !== "") {
					_aApplicationKeys.push(sApplicationKey);
				}
			}
		...
		if (aApplicationKeys.length) {
			...
			oLaunchPageAdapterConfig = _getObject("services.LaunchPage.adapter.config", 0, oConfig);
			...
			oNavTargetResolutionConfig = _getObject("services.NavTargetResolution.adapter.config.applications", 0, oConfig);
	...
	function applyJsonApplicationConfig(sUrlPrefix) {
		...
		request.done(
			function (data) {
				jQuery.sap.log.debug("Evaluating fiori launchpad sandbox config JSON: " + JSON.stringify(data));
				mergeConfig(window["sap-ushell-config"] || {}, data, true);
			});
		request.fail(
			function (oXHRResponse, sTextStatus) {
				if (oXHRResponse.statusCode !== 404) {
					jQuery.sap.log.error("Failed to load Fiori Launchpad Sandbox configuration from " + sUrl + ": status: " + oXHRResponse.status +
	...
	function getBootstrapScriptPath() {
		...
		oBootstrapScript = window.document.getElementById("sap-ushell-bootstrap"); // eslint-disable-line
		if (!oBootstrapScript) {
			// fallback to last script element, if no ID set (should work on most browsers)
			oScripts = window.document.getElementsByTagName("script"); // eslint-disable-line
	...
	function evaluateCustomRenderer(sRenderer) {
		...
		oApplications = _getObject("services.NavTargetResolution.adapter.config.applications", 5, oSapShellConfig);
	...
	function bootstrap(fnCallback) {
		...
		var aConfigFiles = getUriParameters("sap-ushell-sandbox-config", true),
			sCustomRenderer = getUriParameters("sap-ushell-sandbox-renderer"),
			i,
		...
		sap.ui.require(["sap/ushell/services/Container"], function (Container) {
			...
			var sLang = getUriParameters("sap-language"),
				sSufix = "_en";
			if (sLang && sLang.length >= 2) {
				sSufix = "_" + sLang.toLowerCase().substring(0, 2);
			}
			applyJsonApplicationConfig(getBootstrapScriptPath() + "../sandbox/fioriSandboxConfig" + sSufix + ".json");
			...
			if (aConfigFiles && aConfigFiles.length > 0) {
				...
				applyJsonApplicationConfig(getBootstrapScriptPath() + "../appconfig/fioriSandboxConfig.json");
			}

			// the config needs to be adjusted depending on parameter sap-ushell-sandbox-renderer
			evaluateCustomRenderer(sCustomRenderer);

			window["sap-ushell-config"] = adjustApplicationConfiguration(window["sap-ushell-config"]); // eslint-disable-line
			oRendererConfig = _getObject("renderers.fiori2.componentData.config", 0, window["sap-ushell-config"]);
			...
			oUi5ComponentLoaderConfig = _getObject("services.Ui5ComponentLoader.config", 0, window["sap-ushell-config"]);
			...
			oClientSideTargetResolutionConfig = _getObject("services.ClientSideTargetResolution.adapter.config", 0, window["sap-ushell-config"]);
			oClientSideTargetResolutionConfig.applications = _getObject("services.NavTargetResolution.adapter.config.applications", undefined, window["sap-ushell-config"]);
	...
	window["sap-ui-config"] = { // eslint-disable-line
	...
	sap.ushell.__sandbox__ = sap.ushell.__sandbox__ || {}; // eslint-disable-line
	sap.ushell.__sandbox__._adjustApplicationConfiguration = adjustApplicationConfiguration; // eslint-disable-line
	sap.ushell.__sandbox__._applyJsonApplicationConfig = applyJsonApplicationConfig; // eslint-disable-line
	sap.ushell.__sandbox__._evaluateCustomRenderer = evaluateCustomRenderer; // eslint-disable-line
	sap.ushell.__sandbox__._bootstrap = bootstrap; // eslint-disable-line
}());